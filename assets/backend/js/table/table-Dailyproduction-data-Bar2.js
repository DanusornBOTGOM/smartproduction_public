...
async function fetchData() {
    try {
        const url = new URL('http://192.168.1.214:5000/api/combinedDashboardCut');
        const cutUrl = new URL('http://192.168.1.214:5000/api/cutDashboard');
        url.searchParams.append('date', formatDate(selectedDate));
        url.searchParams.append('machineCodePrefix', 'CO2,PAP');
        url.searchParams.append('additionalMachineCodes', 'DRA022,CO2001,CO2002,CO2003,PAP001,PAP002');
        cutUrl.searchParams.append('date', formatDate(selectedDate));

        console.log('Fetching data with URL:', url.toString());
        console.log('Fetching CUT data with URL:', cutUrl.toString());

        const [response, cutResponse] = await Promise.all([
            fetch(url),
            fetch(cutUrl)
        ]);
        
        if (!response.ok || !cutResponse.ok) {
            throw new Error(`HTTP error! status: ${response.status} or ${cutResponse.status}`);
        }
        
        const data = await response.json();
        const cutData = await cutResponse.json();
        console.log('Raw data from API:', data);
        console.log('Raw CUT data from API:', cutData);

        // ดึงข้อมูล Cause
        const causeData = await fetchCauseDataMswAll(selectedDate);
        console.log('Cause data:', causeData);

        if (Array.isArray(data) && Array.isArray(cutData)) {
            allData = [...cutData, ...data].filter(item => 
                (item.MachineCode.startsWith('CUT') && item.MachineCode !== 'CUT022') ||
...
async function showMachineDetails(machineCode, date) {
    try {
        ...
        let detailsHTML = `
            <html>
            <head>
                <title>รายละเอียด ${machineCode} (Bar2)</title>
                <style>
            ...
            <body>
                <h2>รายละเอียดของ ${machineCode} (Bar2)</h2>
        ...
        detailsHTML += `
            ...
            async function saveRemark(docNo, rsnCode, button) {
                ...
                // Check if Remark contains 'CUT' or 'DRA022'
                const cutMatch = remark.match(/CUT\\d{3}/);
                const dra022Match = remark.match(/DRA022/);
                const newMachineCode = cutMatch ? cutMatch[0] : (dra022Match ? 'DRA022' : currentMachineCode);
        ...
        const detailsWindow = window.open('', 'Bar2 Machine Details', 'width=1200,height=600');
        detailsWindow.document.write(detailsHTML);
        detailsWindow.document.close();
    } catch (error) {
        console.error('Error fetching Bar2 machine details:', error);
        alert(`เกิดข้อผิดพลาดในการดึงข้อมูลรายละเอียด Bar2: ${error.message}`);
    }
}

async function saveRemark(docNo, outRSNCode, button) {
    const row = button.closest('tr');
    ...
    if (draMatch) {
        newMachineCode = draMatch[0];
        newSectionID = 'D' + newMachineCode.slice(-2);
    }
...
async function loadPlanData(date, department) {
    try {
        ...
        const response = await fetch(`http://192.168.1.214:5000/api/getPlanDataBar2?date=${date}`);
        ...
        planData = planData.filter(plan => plan.MachineCode !== 'CUT022');
...
function showPlanEditModal(planData, date, department) {
    ...
    // จัดกลุ่มข้อมูลตามประเภทเครื่องจักร
    const groupedData = {
        CUT: planData.filter(plan => plan.MachineCode.startsWith('CUT') || plan.MachineCode === 'DRA022'),
        CO2: planData.filter(plan => plan.MachineCode.startsWith('CO2')),
        PAP: planData.filter(plan => plan.MachineCode.startsWith('PAP'))
    };    
                                    ...
                                    <tr data-no="${plan.No}">
                                        ...
                                        <td>
                                            <select class="form-control" name="machineCode">
                                                ${getMachineOptionsForDepartment('BAR2', plan.MachineCode)}
                                            </select>
                                        </td>
                                        <td>
...
function getMachineOptionsForDepartment(department, selectedMachine = '') {
    // แยกเครื่องจักรเป็นกลุ่มสำหรับ BAR2
    const machineGroups = {
        'BAR2': {
            'CUT': ['CUT001', 'CUT002', 'CUT003', 'CUT004', 'CUT005', 
                    'CUT006', 'CUT007', 'CUT008', 'CUT009', 'CUT010', 
                    'CUT011', 'CUT012', 'CUT013', 'CUT014', 'CUT015', 
                    'CUT016', 'CUT017', 'CUT018', 'CUT019', 'DRA022'],
            'CO2': ['CO2001', 'CO2002', 'CO2003'],
            'PAP': ['PAP001', 'PAP002']
        }
    };

    // ถ้าไม่ใช่ BAR2 หรือไม่พบข้อมูล ให้ return array ว่าง
...
async function saveNewPlan(button) {
    ...
    try {
        ...
        const response2 = await fetch(`http://192.168.1.214:5000/api/getPlanDataBar2?date=${date}`);
...
async function exportToExcel() {
    ...
    const worksheet = workbook.addWorksheet('BAR2 Report');
    ...
    try {
        ...
        link.href = url;
        link.download = `Bar2_report_${selectedDate}.xlsx`;
