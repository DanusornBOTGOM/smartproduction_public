...
async function fetchDataAndPopulateTable() {
    try {
        const urls = [
            createUrlForMachinePrefix(MACHINE_PREFIX_CU),
        ...
        const combinedData = dataArrays.flat().filter(item => 
            (item.MachineCode.startsWith('CUT') && parseInt(item.MachineCode.slice(3)) <= 18) ||
            item.MachineCode === 'DRA022' ||
            (item.MachineCode.startsWith('CO2') && ['CO2001', 'CO2002', 'CO2003'].includes(item.MachineCode)) ||
            (item.MachineCode.startsWith('PAP') && ['PAP001', 'PAP002'].includes(item.MachineCode))
        );
...
function createUrlForMachinePrefix(prefix) {
    ...
    if (prefix === MACHINE_PREFIX_CU) {
        url.searchParams.append('maxMachineNumber', '18');
    } else if (prefix === MACHINE_PREFIX_CO2) {
        url.searchParams.append('specificMachines', 'CO2001,CO2002,CO2003');
    } else if (prefix === MACHINE_PREFIX_PAP) {
        url.searchParams.append('specificMachines', 'PAP001,PAP002');
    }
...
function createTable(data) {
    ...
    const sortedMachines = Object.keys(data).sort((a, b) => {
        const prefixOrder = { 'DRA022': 0, 'CUT': 1, 'CO2': 2, 'PAP': 3 };
        const getPrefix = (code) => {
            if (code === 'DRA022') return 'DRA022';
            if (code.startsWith('CUT')) return 'CUT';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchDataAndPopulateTable();
    document.getElementById('exportProductionDataButton').addEventListener('click', exportProductionDataToExcel);
});
