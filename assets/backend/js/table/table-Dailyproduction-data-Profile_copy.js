...
async function fetchData() {
    try {
        const url = new URL(
            "http://192.168.1.214:5000/api/production/profile/daily"
        );
...
async function fetchBreakdownCauses() {
    try {
        const response = await fetch('/api/production/breakdown-causes');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
...
function saveCause() {
    ...
    fetch('/api/production/saveAllCause', {
        method: 'POST',
        headers: {
...
async function showMachineDetails(machineCode, date) {
    try {
        ...
        const response = await fetch(
            `http://192.168.1.214:5000/api/production/machineDetailsExtended?startDate=${startDate.toISOString()}&endDate=${extendedEndDate.toISOString()}&machineCode=${machineCode}`
        );
        ...
        detailsHTML += `
            ...
            async function saveRemark(docNo, rsnCode, button) {
                ...
                try {
                    const response = await fetch('http://192.168.1.214:5000/api/production/updateRemark', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
...
async function saveRemark(docNo, rsnCode, button) {
    ...
    try {
        const response = await fetch('http://192.168.1.214:5000/api/production/profile/update-remark', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function fetchCauseDataMswAll(date) {
    try {
        const response = await fetch(`http://192.168.1.214:5000/api/production/getCausesMswAll?date=${formatDate(date)}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
...
async function saveProblem(machineCode, date, docNo, problems, deletedProblems) {
    try {
        const response = await fetch('http://192.168.1.214:5000/api/production/profile/update-causes', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function loadPlanData(date, department) {
    try {
        ...
        const response = await fetch(`http://192.168.1.214:5000/api/production/getPlanData?date=${date}&department=${department}`);
...
async function saveNewPlan(button) {
    ...
    try {
        const response = await fetch('http://192.168.1.214:5000/api/production/addNewPlan', {
            method: 'POST',
        ...
        const response2 = await fetch('http://192.168.1.214:5000/api/production/getPlanData?date=${date}&department=${department}');
...
async function deletePlanRow(planNo) {
    if (confirm('ต้องการลบข้อมูลนี้ใช่หรือไม่')) {
        try {
            const response = await fetch('http://192.168.1.214:5000/api/production/deletePlan', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function updatePlanRow(planNo) {  
    ...
    try {
        const response = await fetch('http://192.168.1.214:5000/api/production/updatePlan', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data)
        });
