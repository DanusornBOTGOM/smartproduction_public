...
async function fetchDataAndPopulateTable() {
    try {
        ...
        url.searchParams.append('machineCodePrefix', 'COM,CUT022,TWR'); // สำหรับ BAR1
        ...
        const groupedData = data.reduce((acc, item) => {
            ...
            if (!acc[machineCode][day]) {
                acc[machineCode][day] = { plan: 0, actual: 0, ng: 0 };
            }
            acc[machineCode][day].plan += item.ProductionQuantity || 0;
            acc[machineCode][day].actual += item.Actual || 0;
            acc[machineCode][day].ng += item.NgWeight || 0;
            return acc;
        }, {});
...
function createTable(data) {
    ...
    // Filter and sort machines for BAR1, including CUT022
    const filteredAndSortedMachines = Object.keys(data)
        .filter(machine => machine.startsWith('COM') || machine === 'CUT022' || machine.startsWith('TWR') )
        .sort((a, b) => {
            if (a === 'CUT022') return 1; // Move CUT022 to the end
            if (b === 'CUT022') return -1;
