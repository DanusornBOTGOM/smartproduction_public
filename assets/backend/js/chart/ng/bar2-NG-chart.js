...
function number_format(number, decimals, dec_point, thousands_sep) {
  ...
  async function fetchDataForWasteChart(startDate, endDate) {
      try {
          ...
          // // ตรวจสอบว่า data.summary มีข้อมูลไหม
          // if (!data.summary || data.summary.length === 0) {
          //   console.warn('No summary data received from API');
          //   return [];
          // }

          // // กรองข้อมูลอีกขั้น เผื่อ API ส่งข้อมูลนอกเหนือจากที่ตั้งไว้มา
...
function updateTableDate(startDate, endDate) {
  ...
  if (typeof showMachineDetails === 'function') {
      ...
      const selectedMachine = machineSelect ? machineSelect.value : 'CUT%';
