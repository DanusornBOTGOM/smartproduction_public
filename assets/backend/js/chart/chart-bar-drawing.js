...
function combineData(data) {
  ...
  const result = [{
      MachineCode: 'All Drawing Machines',
...
async function fetchDataForBarChart(startDate, endDate) {
  try {
    ...
    // Remove these lines as they are specific to CUT machines
    // url.searchParams.append('machineCodePrefix', 'CUT');
    // url.searchParams.append('additionalMachineCodes', 'DRA022');
    ...
    // กรองข้อมูลเฉพาะ DRA% โดยไม่เอา DW ของพวกรีดเหลี่ยม
    ...
    // Calculate percentage and sort by MachineCode
  ...
  try {
    ...
    window.myBarChart = new Chart(barChartContext, {
      ...
      data: {
        ...
        datasets: createDatasets(chartData),  // ใช้ฟังก์ชัน createDatasets แทน
      },
      options: {
        responsive: true,
  ...
  if (toggleChartViewButton) {
    toggleChartViewButton.addEventListener('click', function() {
      ...
      document.querySelector('.card-header h6').textContent = isPercentageView ? 
        "%Production On Plan (Drawing)" : "Plan vs Actual Production (Drawing)";
      createOrUpdateBarChart();
    });
  } else {
