function calculateCumulativePOP(data) {
    ...
    const result = Object.entries(groupedData).map(([machineCode, data]) => {
      ...
      const numA = parseInt(a.MachineCode.replace('CGM', ''));  
      const numB = parseInt(b.MachineCode.replace('CGM', '')); 
      return numA - numB;
    });
...
async function generateWeeklyReport(startDate, endDate) {
    try {
        // ดึงข้อมูล NCR ปัจจุบัน
        const response = await fetch(`http://192.168.1.214:5000/api/weeklyReportNCR?startDate=${startDate}&endDate=${endDate}`);
        if (!response.ok) {
        ...
        const filteredData = data
            ...
            .reduce((acc, item) => {
                ...
                if (!existingItem) {
                    ...
                    // ถ้ายังไม่มี PreventiveCorrection ให้ใส่จากข้อมูลที่บันทึกไว้
